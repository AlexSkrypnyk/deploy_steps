...
This is a Deploy Steps template for creating contributed modules or themes. The project provides a complete development environment with CI configuration, testing setup, and deployment automation for publishing to Drupal.org.
...
- `DeployStepsService.php` - Main service class template
...
1. Run `php init.php` to customize template for Deploy Steps
2. Build environment: `make build` or `ahoy build`
3. Develop Deploy Steps code in `src/`
...
- SQLite database created in `/tmp/site_deploy_steps.sqlite`
